...
int sem_create()
{
	...
	if(id >= 0 && id < 30){
		if(semaphores[id] != NULL){
			...
			semaphores[id] = malloc (sizeof(Semaphore));
			semaphores[id]->q = init_queue();
		...
		printf("Please id between 0-29, the id was %d\n", id);
		j = 1;
	}
	return j;
}
...
int sem_terminate()
{
	...
	int id = m_in.m1_i1;
	if(semaphores[id] == NULL){
		printf("Semaphore %d dont exist\n",id); 
		j = -1;
	}
	else{
		semaphores[id]->value = 0;
	       // semaphores[id]->q=NULL;
		semaphores[id] = NULL;		
		printf("Semaphore %d is terminated\n", id);
		j = 0;
	}
	return(j);
}
...
int sem_down()
{
	...
	if(semaphores[id] == NULL){
		...
		if(semaphores[id]->value == 1){
			semaphores[id]->value--;
			printf("Semaphore %d is active with process %d \n \n",
id, pid); 
			semaphores[id]->pid = pid;
			return 0;
		}
		else {
...
int sem_up()
{
	...
	if(semaphores[id] == NULL){
		...
		else if(semaphores[id]->value == 0){
			if(semaphores[id]->pid == pid){
				semaphores[id]->value++;
